...
| `POST /purchase-order/share` \| `confirm` | purchase_orders_send; gating (approved+phone, canShare/canConfirm) then advances the send channel; **the Meta send is MOCKED** (no template pipeline) — buttons behave, sends aren't actually dispatched | Unit |
